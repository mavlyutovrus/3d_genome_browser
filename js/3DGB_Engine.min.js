...
$('#incoords')[0].placeholder=DEFAULT_INCOORD_ENTRY;$("i#name_of_model").text(name_of_model);wrong_chrms_dialog_message="Wrong format of input data. Please, do use the following format:'"+((serviceForStructure=="3d")?"[chr#,]#,#', where the first part is optional":"chr#,#,#'")+". \nSeparator can be any symbol except for numeric one."+((serviceForStructure=="3d")?" In terms of current model, you can use only "+particularChromosome+" chromosomes.":"");startPositionOfCamera=Global2local({x:_startPositionOfCamera.x,y:_startPositionOfCamera.y,z:_startPositionOfCamera.z});}
function changeOptionFoo(indx){switch(indx){case 0:{DEFAULT_INCOORD_ENTRY=((serviceForStructure=='js_test')?'chr17 ':'')+'7571719-7590868';$('#incoords')[0].placeholder=DEFAULT_INCOORD_ENTRY;break;}
case 1:{DEFAULT_INCOORD_ENTRY='TP53';$('#incoords')[0].placeholder=DEFAULT_INCOORD_ENTRY;break;}
case 2:{DEFAULT_INCOORD_ENTRY=((serviceForStructure=='js_test')?'400:150:0':'2.607:3.066:2.406');$('#incoords')[0].placeholder=DEFAULT_INCOORD_ENTRY;break;}}}
...
break;}
case"chr-pos":{var temp=incoords.match(regexpPatternChrms);if(temp){if(particularChromosome&&temp[2]&&temp[2].toUpperCase()!=particularChromosome.toUpperCase()){$("#dialog-message").text(wrong_chrms_dialog_message).dialog("open");return false;}
...
if(controls.isKeyEnabled){switch(event.keyCode){case 66:{if(event.shiftKey&&event.ctrlKey){isDrawingBox=!isDrawingBox;if(isDrawingBox){for(var line in boxLines){scene.add(boxLines[line]);}}else{for(var line in boxLines){scene.remove(boxLines[line]);}}}
break;}
