...
*
*/
vars.getRangesForGenePromoterCubicArea = function(chr, geneStart, geneEnd, isNormal, radiusCube, isPositive) {
	...
	vars.waitFunction(function() {vars.Print_Input_For_Ensembl_Specific(vars.consts.gene_name, vars.final_results, vars.consts.RELEASE_NUM, vars.consts.SEQ_REGION_ID)});
}
...
function getRangesForGenePromoterCubicArea(chr, geneStart, geneEnd, isNormal, radiusCube, isPositive) {
	...
	var getApproximatedRangesStrandsForRadiusFunction = function(meshes, currentPoint, radiusCube, leftPoint, rightPoint, chr) { // where current point is set: [X_new, Y_new, Z_new]
	    ...
	    for (var res in resultRanges) {
			if (resultRanges[res][1]< currentPoint[0] && resultRanges[res][2]>currentPoint[0] ) {
				results[0] = [resultRanges[res][0], ((isPositive)?currentPoint[0]:currentPoint[0]-vars.consts.PROMOTER_LENGTH), ((isPositive)?currentPoint[0]+vars.consts.PROMOTER_LENGTH:currentPoint[0])];
				tempVar = res;
				break;
			}
		}
	...
	var getjson3dCurrentPointFunction = function () {
		...
		function(data) {
			...
			for (var res in finalRanges) {
				print("http://1kgenome.exascale.info/js_snp?chr="+finalRanges[res][0]+"&start="+finalRanges[res][1]+"&end="+finalRanges[res][2]);
			}
...
for (var res in vars.final_results) {
	_req_num++;
	$.getJSON("http://1kgenome.exascale.info/chipseq?chr="+vars.final_results[res][0]+"&start="+vars.final_results[res][1]+"&end="+vars.final_results[res][2]+"&celline=K562" + "?callback=?", null, function(data) {main_set.push(data);_res_num++;})
}
waitFunc();



// !!! if I have complete vars.final_results - want to find genes that are located near (with some intersection) strands (+promoter) in cubic area around promoter region of initial GENE

var bRange=0,eRange=0; 
var setOfGenesInCube = {}; //only intersection or full cover
for (var range in vars.final_results) {
	_chrRange = vars.final_results[range][0];
	bRange = vars.final_results[range][1];
	eRange = vars.final_results[range][2];
...
for (var temp  in setOfGenesInCube) {
	print(temp + " : intersection with" + ((setOfGenesInCube[temp][5])?"":" NOT") + " promoter of RB1 (on the "+setOfGenesInCube[temp][3]+" of range). Length of intersection = " + setOfGenesInCube[temp][4]);
}